...
def main():
    ...
    if args.test_type == 'summ':
        ...
        for ex in test_file:
            """
            source_sent = ex['document']
            summary_sent = ex[args.summary_type]
            """
            source_sent = ex['article_sent']
            summary_sent = ex['correct_sent']
            

            score = score_example_single_context(summary_sent, source_sent, model, tokenizer, args)
            scores.append(score)
        
        print(scores)

    elif args.test_type == 'para':
        test_file = json.load(open('../resources/paraphrase_test.json'))

        scores = []

        for ex in test_file:
            input_sent = ex['input']
            output_sent = ex['positive']
            

            score = score_example_single_context(input_sent, output_sent, model, tokenizer, args)
            scores.append(score)
